...
async def get_recents(request: Request):
    ...
    if user_auth and ("user_id" in user_auth or "email" in user_auth):
        ...
        cursor.execute(
            'SELECT case_id, symptoms, primary_diagnosis, prescribed_formulation, bioactive_match_score, timestamp FROM episodic_cases WHERE patient_id = ? OR patient_id = ? OR patient_id = ? ORDER BY timestamp DESC LIMIT 15',
